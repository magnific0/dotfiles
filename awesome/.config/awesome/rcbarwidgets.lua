...
tagiconnames = { "layers_1", "globe_2", "calendar_2", "doc_edit", "bug", "book", "headphones", "spechbubble_2"}
tagicons = {}
for i = 1, 8 do
...
tags = {
   names  = { "₁", "₂", "₃", "₄", "₅", "₆", "₇", "₈"},
   --mes  = { "term", "net", "org", "write", "arena", "shelf", "media", "link"}
   icons  = tagicons,
...
mailiconbuttonscheme = awful.util.table.join(
   awful.button({ }, 1, function () runner("emacs -gnus --no-desktop") end))
...
vicious.register(mailwidget, 
		 ...
		 function(widget,args)
		    local io  = io.open(homedir .. "/.scripts/mailcheck/mailstatus", "r")
		    if io ~= nil then 
		       local s = io:read()
		       return s
		    else
...
if display_bars then
   ...
   vicious.register( batbar, vicious.widgets.bat, "$2", 20, "BAT1" )
end
...
if display_bars then
   ...
   vicious.register(volbar, vicious.widgets.volume, "$1", 10, "Master" )
...
netact = {nil,
	  ...
	  awful.util.table.join(
	     awful.button({ }, 1, function () runner("sudo openvpn ~/.local/share/openvpn/ninsei.ovpn") end))}
...
--
-- MPD controls
mpd_button_scheme = awful.util.table.join(
   awful.button({ }, 1, function () runnerwu("mpc toggle","music_play") end),
   awful.button({ }, 2, function () runnerwu("mpc stop","music_play") end),
   awful.button({ }, 3, function () runner("ncmpcpp",true) end),
   awful.button({ }, 4, function () runnerwu("mpc prev","music_play") end),
   awful.button({ }, 5, function () runnerwu("mpc next","music_play") end))
...
music_playc:buttons(mpd_button_scheme)
...
music_pausec:buttons(mpd_button_scheme)



vicious.register(music_play, 
		 vicious.widgets.mpd,
		 function (widget, args)
...
for s = 1, screen.count() do
    ...
    right_layout:add(sepl)
    right_layout:add(mailicon)
    right_layout:add(mailwidget)
    right_layout:add(space)
    right_layout:add(sepc)
    ...
    right_layout:add(music_playc)
    right_layout:add(music_pausec)
    right_layout:add(sepr)
...
globalkeys = awful.util.table.join(
    ...
    -- Media keys
    awful.key( {}, "XF86MonBrightnessUp",     function () runner("~/.scripts/backlight.sh +10") end),
    awful.key( {}, "XF86MonBrightnessDown",   function () runner("~/.scripts/backlight.sh -10") end),
    awful.key( {}, "XF86AudioRaiseVolume",   function () runnerwu("amixer -q sset Master 5%+","volumewidget") end),
    awful.key( {}, "XF86AudioLowerVolume",   function () runnerwu("amixer -q sset Master 5%-","volumewidget") end),
    awful.key( {}, "XF86AudioMute",   function () runnerwu("amixer -q sset Master toggle","volumewidget") end),
    awful.key( {}, "XF86AudioPlay",   function () runnerwu("mpc toggle","music_play") end),
    awful.key( {}, "XF86AudioStop",   function () runnerwu("mpc stop","music_play") end),
    awful.key( {}, "XF86AudioPrev",   function () runnerwu("mpc prev","music_play") end),
    awful.key( {}, "XF86AudioNext",   function () runnerwu("mpc next","music_play") end),
    ...
    awful.key({ modkey, "Shift"   }, "F4",     function () runner("alsamixer",true)            end),
...
awful.rules.rules = {
   ...
   -- For class name com-mathworks-util-PostVMInit does NOT work, mathworks instead does work.
    ...
    { rule = { class = "mathworks", name="Figure" },
      properties = { floating = true, ontop = true } },
    { rule = { class = "Gimp", name="Layers" },
      properties = { floating = true, ontop = true } },
    { rule = { class = "Gimp", name="Toolbox" },
      properties = { floating = true, ontop = true, x = 0, y = 0 } },
    { rule = { class = "URxvt", name="ncmpcpp" },
      properties = { tag = tags[1][7], switchtotag = true } },
    { rule = { class = "Emacs", name="gnus" },
      properties = { tag = tags[1][8], switchtotag = false } },
    { rule = { class = "Emacs", name="org-agenda" },
    ...
    { rule = { class = "Firefox", name="- Mozilla Firefox" },
      properties = { tag = tags[1][2], switchtotag = true } },
    { rule = { class = "Chromium", name="- Chromium" },
      properties = { tag = tags[1][2], switchtotag = true } },
